                ...
                >
                    ...
                    <div class="relative z-20 text-center">
                        ...
                        <p class="mt-5 text-sm text-gray-600">
                            ...
                            folosind drag and drop sau
                            <input class="hidden" type="file" ref="fileInput" @change="droppedFiles($event)" accept="image/*, .pdf" multiple />
                            <button @click="$refs.fileInput.click()" class="text-pink-600 hover:text-pink-500 focus:outline-none focus:underline transition duration-150 ease-in-out">urcându-le de aici</button>
                        </p>
                        <p class="mt-1 text-xs text-gray-500">
                            .png, .jpg, .jpeg, .pdf
                            <br/>
                            Dacă dorești să ștergi un document apasă click pe el.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-md mt-5">
            <ul>
                <li v-for="(file, index) in form.files" :key="index">
                    <a @click="removeFile(file)" class="block hover:bg-gray-50 focus:outline-none focus:bg-gray-50 transition duration-150 ease-in-out">
                        <div class="px-4 py-4 sm:px-6">
                            <div class="flex items-center justify-between">
                                <div class="text-sm leading-5 font-medium text-pink-600 truncate">{{ file.name }} ({{ file.size }})</div>
                                <div class="ml-2 flex-shrink-0 flex">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">Încărcat, gata de trimis</span>
                                </div>
                            </div>
                        </div>
                    </a>
...
export default {
    ...
    methods: {
        ...
        droppedFiles(e) {
            let droppedFiles;
            if (e.toString() === "[object DragEvent]") {
                droppedFiles = e.dataTransfer.files;
            } else {
                droppedFiles = this.$refs.fileInput.files;
            }
        ...
        uploadFiles() {
            this.form.post(route('admin.auto.upload-documents', this.$props.editable.id))
